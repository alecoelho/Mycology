    ...
    public partial class Exame : Form
    {
        ...
        private void LoadForm(int pCodigo)
        {
            ...
            if (entity != null)
            {
                ...
                txtNumeroExame.Text = codigo.ToString();
        ...
        private void Salvar()
        {
            ...
            entity.nNumeroExame = int.Parse(txtNumeroExame.Text);
                ...
                else
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        if (MessageBox.Show("Inclusão Efetuada. Imprimir Resultado?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                            ImprimirResultado(entity.nNumeroExame);
                    }
                    else
        ...
        {
            util.ExecutarSqlCrud("DELETE FROM MICOLOGIA_ExameResultado WHERE nNumeroExame = " + entity.nNumeroExame.ToString());
            ...
            if (dt != null)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    exameResultado.nNumeroExame = entity.nNumeroExame;
        ...
        private bool Criticar()
        {
            ...
            {
                MessageBox.Show("Número de Exame já existente", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        ...
        {
            if (blExame.ListarPorChave(int.Parse(txtNumeroExame.Text)).Count == 0)
                return false;
        ...
        {
            IList<Modelo.MICOLOGIA_Exame> lista = blExame.ListarTodos();

            if (lista.Count != 0)
                return lista[lista.Count - 1].nNumeroExame + 1;
