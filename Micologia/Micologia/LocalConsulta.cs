...
namespace Micologia
{
    public partial class LocalConsulta : Form
    {
        IList<MICOLOGIA_Local> lista;

        public LocalConsulta()
        {
            InitializeComponent();

            txtPesquisa.Select();

            if ((from item in PermissaoUsuarioBL.ListaPermissao where item.Tela.sDescricao.Trim() == "Local" && item.sIncluir.Trim() == "N" select item).Count() > 0)
        ...
        private void Pesquisar()
        {
            ...
            LocalBL blPaciente = new LocalBL();
        ...
        private void CarregarGrid()
        {
            ...
            if (lista.Count > 0)
            {
                ...
                {
                    dgvConsulta.Rows.Add("Alterar", lista[i].nIDLocal.ToString(), lista[i].sDescricao);
                }
            }
            else
        ...
        private void gerarColunaDataGridView()
        {
            ...
            dgvConsulta.Columns.Add("local", "Local");

            dgvConsulta.Columns[1].Visible = false; //codigo
            dgvConsulta.Columns[2].Width = 350; //resultado


            if ((from item in PermissaoUsuarioBL.ListaPermissao where item.Tela.sDescricao.Trim() == "Local" && item.sAlterar.Trim() == "N" select item).Count() > 0)
                dgvConsulta.Columns[0].Visible = false;
        }

        #region Singleton
        private static LocalConsulta form = null;
        public static LocalConsulta getInstance()
        {
            if (form == null)
                form = new LocalConsulta();
